...
export default function Header() {
    ...
    useEffect(() => {
        ...
        const handleStorage = (event: StorageEvent) => {
            ...
            if (
                event.key === `avatar_color_${currentUserId}` &&
                event.newValue
            ) {
        ...
        window.addEventListener(
            "avatarColorUpdated",
            handleAvatarColorUpdate as EventListener,
        );
    ...
    const fetchNotifications = async (userId: string, showLoading = true) => {
        ...
        try {
            const response = await fetch(
                `${apiBase}/users/${userId}/notifications`,
            );
        ...
        <header className="w-full py-4 px-4 sm:px-6 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
            <div className="flex flex-row justify-between w-full items-center">
                <div className="flex items-center gap-2 sm:gap-3">
                    <Link
                        href={logoHref}
                        className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-blue-600 via-purple-500 to-pink-500 bg-clip-text text-transparent hover:opacity-80 transition-opacity"
                    >
                        AistM
                    </Link>
