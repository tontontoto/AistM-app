    ...
    const handleAddSkill = (projectId: number) => {
        ...
        setData(prev => {
            ...
            const projects = prev.projects.map(project => {
                ...
                return { ...project, skills_required: [...project.skills_required, input] };
            });
    ...
    const handleRemoveSkill = (projectId: number, name: string) => {
        ...
        setData(prev => {
            ...
            const projects = prev.projects.map(project => {
                ...
                return {
                    ...project,
                    skills_required: project.skills_required.filter(skill => skill !== name),
                };
            });
            return { ...prev, projects };
        });
        ...
        return (
            <div className="w-full">
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    <p className="text-red-600">エラー: {error || "データを取得できませんでした"}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full">
            <div className="mb-6 sm:mb-8">
                <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">PM チーム・ヘルス・ダッシュボード</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    ...
                    <p className="text-xs text-gray-400 mt-1">進行中で滞留しているタスク</p>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <p className="text-xs text-gray-500">管理プロジェクト</p>
                    <p className="text-3xl font-bold text-blue-600 mt-2">{data.projects.length}</p>
            ...
            <div className="space-y-6">
                {data.projects.map(project => (
                    <div key={project.id} className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                            <div>
                                <h2 className="text-lg sm:text-xl font-semibold text-gray-800">{project.overview}</h2>
                                <p className="text-xs text-gray-500">メンバー: {project.members_count}人</p>
                            </div>
                            <div className="flex items-center gap-2 text-xs">
                                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full">{project.status?.name || "未設定"}</span>
                                <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded-full">{project.priority?.name || "未設定"}</span>
                                <span className="px-2 py-1 bg-red-100 text-red-700 rounded-full">SOS {project.sos_count}</span>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                            <div className="bg-gray-50 rounded-xl p-4">
                                <p className="text-sm font-semibold text-gray-700 mb-2">スキル・カバレッジ</p>
                                <div className="flex items-center gap-3 mb-2">
                                    <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                        <div
                                            className="h-2 bg-green-500"
                                            style={{ width: `${project.skill_coverage.percent}%` }}
                                        ></div>
                                    </div>
                                    <span className="text-xs text-gray-600">{project.skill_coverage.percent}%</span>
                                </div>
                                <p className="text-xs text-gray-500">
                                    充足: {project.skill_coverage.covered}/{project.skill_coverage.total_required}
                                </p>
                                {project.skill_coverage.missing.length > 0 && (
                                    <div className="mt-2 text-xs text-orange-600">
                                        未充足: {project.skill_coverage.missing.join(", ")}
                                    </div>
                                )}

                                <div className="mt-3">
                                    <p className="text-xs font-semibold text-gray-600 mb-2">必要スキルの編集</p>
                                    <div className="flex gap-2 mb-2">
                                        <input
                                            type="text"
                                            value={skillInputs[project.id] || ""}
                                            onChange={(event) =>
                                                setSkillInputs(prev => ({ ...prev, [project.id]: event.target.value }))
                                            }
                                            onKeyDown={(event) => {
                                                if (event.key === "Enter") {
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => handleAddSkill(project.id)}
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-2">
                                        {project.skills_required.length === 0 ? (
                                            <span className="text-xs text-gray-400">スキル未登録</span>
                                        ) : (
                                        ...
                                        {project.stalled_tasks.map(task => (
                                            <div key={task.id} className="bg-white border border-gray-200 rounded-lg p-3">
                                                <div className="flex items-center justify-between">
                                                    <div>
                                                        <p className="text-sm font-semibold text-gray-800">{task.overview}</p>
                                                        <p className="text-xs text-gray-500">期限: {task.schedule}</p>
                                                    </div>
                                                    <div className="text-xs text-gray-500 text-right">
                                                        <p>閾値: {task.halfway_at}</p>
                                                        <p>最終Help: {task.last_help_at || "未送信"}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
